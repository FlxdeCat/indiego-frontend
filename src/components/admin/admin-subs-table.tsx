...
function getSubscriptionTableColumns(nav: ReturnType<typeof useNavigate>, setRejectRefund: React.Dispatch<React.SetStateAction<number | null>>): ColumnDef<z.infer<typeof schema>>[] {
  return [
    {
      accessorKey: "cover",
      header: () => (
...
export function AdminSubsTable({
  data: initialData,
}: {
  ...
  const [rejectRefundIndex, setRejectRefund] = React.useState<number | null>(null)

  const nav = useNavigate()
  const columns = getSubscriptionTableColumns(nav, setRejectRefund)
  ...
  return (
    ...
    //   <DeleteGameDialog
    //     open={rejectRefundIndex !== null}
    //     title={rejectRefundIndex !== null ? data[rejectRefundIndex].title : ""}
    //     onOpenChange={(open) => {
    //       if (!open) setRejectRefund(null)
    //     }}
    //   />
    // </>
  )
